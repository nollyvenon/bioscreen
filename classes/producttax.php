...
class ProductTax {
	...
	function add_tax($tax_name, $tax_rate, $tax_type, $tax_description) { 
		...
		if($num_rows > 0) { 
			...
			$query = "INSERT into product_taxes(tax_id,tax_name,tax_rate,tax_type,tax_description,store_id) VALUES(NULL, '".$tax_name."', '".$tax_rate."', '".$tax_type."', '".$tax_description."', '".$_SESSION['store_id']."')";
			$result = $db->query($query) or die($db->error);
