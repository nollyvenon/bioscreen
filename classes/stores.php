...
class Store {
	...
	function update_store($store_id, $store_manual_id, $store_name, $business_type, $address1, $address2, $city, $state, $country, $phone, $email, $currency, $store_logo, $description) {
		global $db;
		if($_SESSION['user_type'] != 'admin') {
			exit();
		}//checks admin user.
	...
	function add_store($store_manual_id, $store_name, $business_type, $address1, $address2, $city, $state, $country, $phone, $email, $currency, $store_logo, $description) { 
		global $db;
		...
		if($num_rows > 0) { 
			...
			(store_id, store_manual_id, store_name, business_type, address1, address2, city, state, country, phone, email, currency, store_logo, description, user_id) 
			VALUES(NULL, "'.$store_manual_id.'", "'.$store_name.'", "'.$business_type.'", "'.$address1.'", "'.$address2.'", "'.$city.'", "'.$state.'", "'.$country.'", "'.$phone.'", "'.$email.'", "'.$currency.'", "'.$store_logo.'", "'.$description.'", "'.$_SESSION['user_id'].'")';
			$result = $db->query($query) or die($db->error);
