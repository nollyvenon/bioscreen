...
class Users {
	...
	function register_user($first_name, $last_name, $user_type, $username, $email, $password){
			...
			$curtime = date('Y-m-d');
			$query = "INSERT INTO users(user_id, first_name,last_name,username,email,password,activation_key,date_register,user_type,status) VALUES (NULL, '$first_name', '$last_name', '$username', '$email', '$password', '$activation_key', '$registration_date', '$user_type', '$status', '$curtime')";
			$result = $db->query($query) or die($db->error);
		...
		function facebook_login_register($first_name, $last_name, $gender, $email, $user_type) {
		 ...
		 if($num_rows == 0) {
			 $curtime = date('Y-m-d');
		 	$query = "INSERT INTO users(user_id, first_name,last_name,gender,email,password,date_register,user_type,status) VALUES (NULL, '$first_name', '$last_name', '$gender', '$email', '$password', '$registration_date', '$user_type', '$status', '$curtime')";
...
function forgot_user($email){
	...
	function add_user($first_name, $last_name, $gender, $date_of_birth, $address1, $address2, $city, $state, $country, $zip_code, $mobile, $phone, $username, $email, $password, $profile_image, $description, $status, $user_type) { 
			...
			$query = "INSERT into users VALUES(NULL, '".$first_name."', '".$last_name."', '".$gender."', '".$date_of_birth."', '".$address1."', '".$address2."', '".$city."', '".$state."', '".$country."', '".$zip_code."', '".$mobile."', '".$phone."', '".$username."', '".$email."', '".$password_con."', '".$profile_image."', '".$description."', '".$status."', '', '".date('Y-m-d')."', '".$user_type."', '".$curtime."')";
			$result = $db->query($query) or die($db->error);
			//Email to user
			$site_url = get_option('site_url');
