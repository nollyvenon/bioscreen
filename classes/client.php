...
class Client {
	...
	function add_client($full_name, $business_title, $mobile, $phone, $age, $sex, $blood_group, $birth_date, $HMO_name, $address, $city, $state, $zipcode, $country, $email, $price_level, $notes) {
		global $db;
		...
		if($num_rows > 0) { 
			return 'A patient with same name already exists.';
		} else { 
			$query = "INSERT into clients(client_id, full_name, business_title, mobile, phone, age, sex, blood_group, birth_date, HMO_name, address, city, state, zipcode, country, email, price_level, notes, store_id)
				VALUES(NULL, '".$full_name."', '".$business_title."', '".$mobile."', '".$phone."', '".$age."', '".$sex."', '".$blood_group."', '".$birth_date."', '".$HMO_name."', '".$address."', '".$city."', '".$state."', '".$zipcode."', '".$country."', '".$email."', '".$price_level."', '".$notes."', '".$_SESSION['store_id']."')
			";
			$result = $db->query($query) or die($db->error);
			$_SESSION['cn_id'] = $db->insert_id;
			return 'Patient added successfuly.';
		}
