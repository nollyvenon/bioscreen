			...
			if($rows > 0) { 
				...
				foreach($access_to as $access) { 
					if($access == 'sales') { 
						...
						$expenses = 1;
					} else if($access == 'labsamples') { 
						$labsamples = 1;
					} else if($access == 'lab_scientists') { 
						$lab_scientists = 1;
					}
				}
				
				$query = "INSERT into store_access(user_id, store_id, sales, purchase, vendors, clients, products, warehouse, returns, price_level, reports, expenses, labsamples, lab_scientists) VALUES('".$user_id."', '".$store_id."', '".$sales."', '".$purchase."', '".$vendors."', '".$clients."', '".$products."', '".$warehouse."', '".$returns."', '".$price_level."', '".$reports."', '".$expenses."', '".$labsamples."', '".$lab_scientists."')";
				$result = $db->query($query) or die($db->error);
	...
	function list_store_access() { 
		...
		if($_SESSION['user_type'] != 'admin') {
			...
			while($row = $result->fetch_array()) {
				...
				if($row['lab_scientists'] == '1'){ 
					$options .= 'Lab Scientists';
				}				
				$options .= '</td>';
